...
import { ChevronLeft, Loader2, Search, Check, Trash2, Calendar } from 'lucide-react';
...
export default function QueryDetailPage({ params }: PageProps) {
  ...
  // Autofill owner from session for new queries
  useEffect(() => {
    const userEmail = session?.user?.email;
    if (isNew && userEmail && !formData.owner) {
      setFormData(prev => ({ ...prev, owner: userEmail }));
    }
  }, [isNew, session?.user?.email, formData.owner]);
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (isNew) {
        ...
        res = await updateQuery(id, payload);
      }
  ...
  const handleDelete = async () => {
    ...
    setLoading(true);
    const res = await deleteQuery(id) as any;
    setLoading(false);
    if (res.success) {
      router.push('/qreg/query');
      router.refresh();
        ...
        <div className="flex items-center gap-3">
          ...
          {!isNew && (
            <button
              ...
              className="px-6 py-2.5 bg-primary text-primary-foreground rounded-md text-xs font-bold hover:brightness-110 transition-all shadow-sm flex items-center gap-2"
            >
              <Calendar size={14} /> Itinerary
            </button>
          )}

          <button type="submit" form="queryForm" disabled={loading} className="px-10 py-2.5 bg-primary text-primary-foreground rounded-md text-xs font-bold hover:brightness-110 disabled:opacity-50 transition-all shadow-sm">
        ...
        <div className="bg-white flex flex-col divide-y divide-zinc-100 h-full">
          ...
          <div className="grid grid-cols-3 items-center p-4 gap-4">
            ...
            <input
              ...
              placeholder={isNew ? "Detecting owner..." : ""}
              className="col-span-2 bg-zinc-50 border border-zinc-200 rounded py-1.5 px-3 text-sm outline-none cursor-not-allowed text-zinc-500 font-medium"
            />
          </div>
